...
const Movie: NextPage<Props> = ({movie, directors, cast}) => (
    <main>
        <Header />
        <Breadcrum title={ movie.original_title } />
        <MovieInfo 
...
export const getStaticProps: GetStaticProps = async context => {
    ...
    const creditsEndpoints: string = creditsUrl( id )
    
    const movie = await basicFetch<Movie>( movieEndpoint )
    const credits = await basicFetch<Credits>( creditsEndpoints )
